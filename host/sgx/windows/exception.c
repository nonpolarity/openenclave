 ...
 * oe_sgx_td_t, stack-protector etc. Simulation mode correctly sets up the FS
 * register prior to making an ecall (see host/sgx/enter.c). Windows however
 * restores the FS register to its original value, possibly whenever the thread
 * resumes execution. This causes crashes since the enclave code expects FS to
 * point to oe_sgx_td_t whereas Windows thread scheduler has reverted it. The FS
 * values Windows uses point to invalid memory and thus enclave code that uses
 * FS register results in access violations. The handler below restores FS to
 * desired value when such a violation occurs.
 */
